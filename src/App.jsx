...
import {
  ...
  generatePlan, generateMarathonPlan,
...
export default function App() {
  const [setup, setSetup] = useState({
    ...
    maxHR: 188,
    restingHR: 50,
    raceDate: '2026-10-25',  // Marine Corps Marathon
  ...
  const handleGenerate = () => {
    ...
    if (!plan) {
      const newPlan = generateMarathonPlan({
        raceDate: setup.raceDate,
    ...
    // Regenerate from the future start date toward race day
    const futureWeeksRaw = generateMarathonPlan({
      raceDate: setup.raceDate,
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      {/* Bottom nav */}
      {(plan || view === 'weather' || view === 'health' || view === 'strength') && (
...
function SetupForm({ setup, onChange, onGenerate, onReset, onBackup, onRestore, hasPlan, onShowMethod }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-5 max-w-2xl">
      ...
      <p className="text-sm text-slate-500 mb-5">Race-driven marathon plan: Phase 1 base + 16-week marathon block ending on race day.</p>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          <p className="text-xs text-amber-900 leading-relaxed">
            <span className="font-semibold">Weekly structure:</span> 4 runs (Sun long · Tue sub-T · Wed easy · Fri sub-T in Phase 2, easy in Phase 1).
            Thursday is open for cross-training or rest. Mon and Sat are rest days.
            Long run capped at 3 hours per Sirpoc.
          </p>
        </div>
      </div>
...
function StrengthView({ rehabConfig, rehabLog, onToggleRehab, onUpdateRehabConfig }) {
  return (
    <div>
      <div className="mb-4">
        <h2 className="text-base font-semibold text-slate-900 flex items-center gap-2">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Track strength &amp; rehab exercises by day</p>
      </div>
      <RehabSection
        config={rehabConfig}
        rehabLog={rehabLog}
