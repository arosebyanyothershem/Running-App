...
// Calibrated to Daniels' published VDOT tables for 5K times:
//   VDOT 70 → 14:27 = 867
//   VDOT 65 → 15:22 = 922
//   VDOT 60 → 16:27 = 987
//   VDOT 55 → 17:45 = 1065
...
export function vdotFrom5K(fiveKseconds) {
  const ref = [
    [70, 867], [65, 922], [60, 987], [55, 1065],
    [52, 1124], [50, 1165], [48, 1208], [46, 1255],
    [45, 1279], [44, 1305], [43, 1333], [40, 1427],
    [35, 1604], [30, 1840],
  ];
...
// Calibrated to Daniels' published VDOT tables.
// Marathon time vs VDOT table (in seconds):
//   VDOT 70 → 2:23:10 = 8590
//   VDOT 65 → 2:33:28 = 9208
//   VDOT 60 → 2:46:08 = 9968
//   VDOT 55 → 3:01:39 = 10899
//   VDOT 50 → 3:21:00 = 12060
//   VDOT 45 → 3:46:31 = 13591
//   VDOT 40 → 4:16:03 = 15363
//   VDOT 35 → 4:53:53 = 17633
// We use linear interpolation between known points.
export function projectMarathonTime(vdot) {
  const ref = [
    [70, 8590], [65, 9208], [60, 9968], [55, 10899],
    [50, 12060], [45, 13591], [40, 15363], [35, 17633], [30, 20580],
  ];
...
export function generateMarathonPlan({ raceDate, startDate, paces, hr, vdot }) {
...
// Phase 1 week: 4 runs (Sun long, Tue sub-T, Wed easy, Fri easy)
...
function buildPhase1Week({
  ...
  weekStart, isStepback,
}) {
  const days = emptyDays(weekStart);

  // Long run: 30% of total weekly miles, capped at 3 hours (~18 mi at 10:00/mi pace)
  const longMiles = clampLongRun(Math.round(totalMiles * 0.32 * 2) / 2, paces);
  // Sub-T workout: separate from mileage calc (workout itself is ~6 mi)
  const subTworkoutMiles = 6;
  // Remaining weekly mileage goes to 2 easy runs
  const easyTotal = Math.max(6, totalMiles - longMiles - subTworkoutMiles);
  const easyMiles = Math.round((easyTotal / 2) * 2) / 2;
  ...
  days[1].sessions.push(buildSubT(weekIndex, false, paces, hr, { conservative }));
  ...
  // Daily activation + pre-run warmup on running days
  addActivationAndWarmup(days, weekIndex);
  ...
  return {
    ...
    subTcount: 1,
    isStepback,
    phase: isStepback ? 'Phase 1 · Step-back' : 'Phase 1 · Base',
    weekStart,
    days,
  };
}
...
function buildPhase2Week({
  ...
  weekStart, block, isStepback, isRaceWeek,
}) {
  const days = emptyDays(weekStart);
  ...
  const tuesdayWorkout = pickTuesdayWorkout(block, blockWeekIndex, paces, hr, mpSecPerMi);
  const fridayWorkout = pickFridayWorkout(block, blockWeekIndex, paces, hr, mpSecPerMi);
  ...
  addActivationAndWarmup(days, totalWeekIndex);
...
// Cap long run at ~3 hours (per Sirpoc).
// At the user's easy pace (e.g., 9:45/mi), that's ~18.5 mi.
function clampLongRun(miles, paces) {
  // Use the slower end of easy pace (high) for conservative time estimate
  const sec = paces.easy.high * miles;
  const maxSec = 3 * 3600;
  if (sec > maxSec) {
    return Math.floor((maxSec / paces.easy.high) * 2) / 2;
  }
  return miles;
}
...
function pickTuesdayWorkout(block, blockWeekIdx, paces, hr, mpSecPerMi) {
  // Rotate Sirpoc's three sub-T staples by week index
  return buildSubT(blockWeekIdx, false, paces, hr);
}

// Friday workout — sub-T in mb1/mb2, alternates with MP work in mb3
function pickFridayWorkout(block, blockWeekIdx, paces, hr, mpSecPerMi) {
  if (block === 'mb3') {
    // Mini-block 3: alternate sub-T and MP-focused workouts
    // Week 10: sub-T
    // Week 11: MP-focused (5×5km at MP)
    // Week 12: sub-T (volume peak)
    // Week 13: MP-focused (3×8km progressive at MP)
    if (blockWeekIdx === 11) {
...
// Sub-T workout — rotates Sirpoc's three staples based on week index.
// `weekContext` lets us prescribe shorter, conservative workouts for the
// very first weeks of a build (Phase 1 introduction).
function buildSubT(weekIndex, recovering, paces, hr, opts = {}) {
  let structure;
  let structured;
  let totalMiles;

  // Phase 1 first two weeks: ease in with 3×1mi (the workout we know Avi tolerates)
  if (opts.conservative) {
    structure = '3 × 1 mi @ sub-T pace w/ 90 sec jog recovery';
    structured = { reps: 3, distM: 1609, recoverySec: 90 };
    totalMiles = 6;
  } else {
    // Sirpoc's three staples (translated to miles):
    //   3×3200m → 3 × 2 mi (the "long reps")
    //   6×1600m → 6 × 1 mi (the "classic mile reps")
    //   10×1000m → 10 × 0.6 mi (the "short reps")
    const rotation = weekIndex % 3;
    if (rotation === 0) {
      // Long reps
      structure = '3 × 2 mi @ sub-T pace w/ 2 min jog recovery';
      structured = { reps: 3, distM: 3219, recoverySec: 120 };
      totalMiles = 8;
    } else if (rotation === 1) {
      // Mile reps
      structure = '6 × 1 mi @ sub-T pace w/ 90 sec jog recovery';
      structured = { reps: 6, distM: 1609, recoverySec: 90 };
      totalMiles = 8;
    } else {
      // Short reps
      structure = '10 × 1 km @ sub-T pace w/ 60 sec jog recovery';
      structured = { reps: 10, distM: 1000, recoverySec: 60 };
      totalMiles = 8;
    }
  }
  ...
  return {
    ...
    hr: `HR ${hr.subTLow}–${hr.subTHigh}`,
    miles: totalMiles,
    detail: `${structure}. Warm-up 1 mi + cool-down 1 mi easy. Should feel firm but controlled — comfortably hard, not gasping.`,
...
function addActivationAndWarmup(days, totalWeekIndex) {
  // Daily AM activation (5 min)
  days.forEach((d, i) => {
    d.sessions.unshift({
      id: `act-${totalWeekIndex}-${i}`,
      type: 'activation',
      title: 'AM Activation (5 min)',
      detail: 'First thing in the morning. Side-lying leg raises 30/side · Clamshells 30/side · Single-leg glute bridges 10/side × 2.',
    });
  });

  // Pre-run warmup on run days
  days.forEach((d, i) => {
    const hasRun = d.sessions.some(s => ['easy', 'subT', 'long'].includes(s.type));
    if (hasRun) {
      d.sessions.splice(1, 0, {
        id: `warm-${totalWeekIndex}-${i}`,
        type: 'warmup',
