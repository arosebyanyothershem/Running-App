...
function buildPhase1Week({
  ...
  if (hasFridaySubT) {
    ...
    days[4].sessions.push(buildSubT(weekIndex + 1, false, paces, hr, { conservative }));
  } else {
...
// At the user's fitness (~VDOT 45-46, sub-T HR 160-170, pace ~7:20-7:55/mi):
//   Short  → 8 × 800m (each rep ~3:30 at ~7:20-7:30/mi)
//   Medium → 5 × 1200m (each rep ~5:30 at ~7:25-7:35/mi)
//   Long   → 3 × 1 mi (each rep ~7:40-7:55 at ~7:40-7:55/mi)
...
function buildSubT(weekIndex, recovering, paces, hr, opts = {}) {
  ...
  // Phase 1 first two weeks: ease in with conservative 3×1mi reentry
  if (opts.conservative) {
    structure = '3 × 1 mi @ sub-T HR w/ 90 sec jog recovery';
    ...
    if (rotation === 0) {
      ...
      // Medium reps
      structure = '5 × 1200m @ sub-T HR w/ 75 sec jog recovery';
      structured = { reps: 5, distM: 1200, recoverySec: 75 };
      totalMiles = 6;
    } else {
      // Short reps
      structure = '8 × 800m @ sub-T HR w/ 60 sec jog recovery';
      structured = { reps: 8, distM: 800, recoverySec: 60 };
      totalMiles = 7;
    }
  }
